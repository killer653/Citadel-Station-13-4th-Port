				...
				if(!H.w_uniform)
					...
					if(!itemstorevalue)
						switch(w_class)
							...
							else itemstorevalue += 10 // This should fix that problem.
					if(itemstorevalue != -1)//if the item is not too big
						if(B.stored < B.capacity && itemstorevalue <= buttspace) // if the stomach can still hold an item
							if(H == user)
								...
								H.visible_message("<span class='warning'>[user] attempts to stuff \the [src] into [H]'s throat...</span>", "<span class='warning'>You attempt to stuff \the [src] into [H]'s throat...</span>")
								if(!do_mob(user, H))
									if(H == user)
										user << "<span class='warning'>You fail to stuff \the [src] down your throat.</span>"
									else
										user << "<span class='warning'>You fail to stuff \the [src] down [H]'s throat.</span>"
									return 0
								H.visible_message("<span class='danger'>[user] stuffs \the [src] into [H]'s throat.</span>", "<span class='userdanger'>You stuff \the [src] into [H]'s throat.</span>")
							user.unEquip(src)
							...
							if(B.stored > B.capacity) // added this due to a trash item being inserted inside without any check, so stuff won't break,i am unsure on how useful this is
								...
								H << "<span class='warning'>Your stomach cannot hold all those items, and one falls out!</span>"
							return 1
						else
							if(H == user)
